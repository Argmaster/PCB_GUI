...
LAYER_TYPES = {
    "COPPER": {
        ...
        "dark_material": {"color": [0, 23, 0, 1.0], "roughness": 1.0, "specular": 0},
        "clear_material": {"color": [0, 76, 0, 1.0], "roughness": 1.0, "specular": 0},
        "region_material": {"color": [0, 76, 0, 1.0], "roughness": 1.0, "specular": 0},
    },
    "SILK": {
        "dark_thickness": "0.1m",
        "clear_thickness": "0.05m",
        ...
        "clear_material": {"color": [0, 0, 0, 1.0], "roughness": 1.0, "specular": 0},
        "region_material": {"color": [255, 0, 0, 1.0], "roughness": 1.0, "specular": 0},
    },
}
...
def renderGerberLayer(io_in: IO_IN, io: BlenderIO):
    ...
    for progress in parser:
        if (progress % 3 == 0):
            io.write(IO_OUT("STREAM", {"tokens_done": 3}))
    Object.join(backend.ROOT, *Global.getAll())
    Global.Export(f"./temp/{io_in.data['layer_id']}.glb")
    return IO_OUT("END")
